...
def kalman_filter(speed, cam_available, cam_data, x_est_prev, P_est_prev, Q, R, Ts):
    ...
    # Constants
    distance_between_wheels = 45  # [mm], in reality distance is only 45mm
...
def control_law(sensor_data, checkpoints_data, current_checkpoint_index, fsm_state, state_kalman, Thymio_found):
    ...
    base_translational_velocity = 150  # thymio speed
    ...
    if fsm_state == "moving":
        ...
        if delta_angle > angle_threshold_low:
            ...
            right_velocity = base_rotational_velocity

            # # higher speed if we are far away
            ...
            if (euler_distance <= distance_threshold) and not last_checkpoint_bool:
                new_checkpoint_index = current_checkpoint_index + 1
